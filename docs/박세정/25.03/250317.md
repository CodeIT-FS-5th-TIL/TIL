...
JWT는선 택적 서명 및 선택적 암호화를 사용하여 데이터를 만들기 위한 인터넷 표준 방식의 토큰입니다.  일반적으로 사용자가 시스템에 인증된 후 지속적인 세션 유지 및 서버 리소스를 절약하는 데 유용합니다. 사용자가 로그인하면 서버는 JWT 토큰을 발급하고, 이 토큰을 클라이언트(브라우저)에서 저장한 뒤 이후 요청 시 이 토큰을 함께 보내어 인증을 처리합니다.
